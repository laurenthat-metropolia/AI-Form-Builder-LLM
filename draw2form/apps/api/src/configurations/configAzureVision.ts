...
const sendImageUrlForTextRecognition = async (
  imageUrl: string
): Promise<string | null> => {
  ...
  console.log(
    `Azure Api | sendImageUrlForTextRecognition:  status: ${response.status}`
  );
...
const checkForTextRecognitionResults = async (operationUrl: string) => {
  try {
    ...
    console.log(
      `Azure Api | checkForTextRecognitionResults:  status: ${response.status}`
    );
    const hasGoodStatus = [200, 201, 202].includes(response.status);
    if (!hasGoodStatus) {
      return null;
...
export const recognizeText = async (
  imageUrl: string
): Promise<TextDetectionResponse | null> => {
  try {
    ...
    return lines.map((line) => {
      const xCoordinates = line.boundingBox.filter(
        (c, index) => index % 2 === 0
      );
      const yCoordinates = line.boundingBox.filter(
        (c, index) => index % 2 !== 0
      );
