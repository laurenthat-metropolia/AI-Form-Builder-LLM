    ...
    useEffect(() => {
...

        {
            ...
            const polygons: CanvasAnnotation[] =
                ev?.parsedPayload?.map(({ text, coordinates }): CanvasAnnotation => {
                    return {
                        type: 'rect',
                        color: 'red',
                        payload: {
        ...
        {
            const ev = uploadedFile.events?.find((event) => event.event === 'PREDICTIONS_UNIFIED') as
                | ApiUnifiedPredictionEvent
                | undefined;
            ...
            const groupedByYAxis = ev?.parsedPayload.reduce(
            ...
            const polygons: CanvasAnnotation[] =
                ev?.parsedPayload?.map(({ type, coordinates, data }): CanvasAnnotation => {
                    return {
                        type: 'rect',
                        color: 'cyan',
                        payload: {
