...
import {
    ...
    recognizeObjects,
} from '../configurations/configObjectDetection.js';
import { ImageEvents } from '../enums.js';
import {
    recognizeText,
    TextDetectionResponse,
    TextDetectionResponseItem,
} from '../configurations/configAzureVision.js';
...
export const processUploadedFile = async (uploadedFile: UploadedFile) => {
    ...
    if (objectDetectionResponses && textDetectionResponse) {
        const unifiedDetections = unifyDetections(objectDetectionResponses, textDetectionResponse);
        ...
        const structureGenerationResponse = await generateFormStructure(chatGPTInput);
...
export function unifyDetections(objects: ObjectDetectionResponse, texts: TextDetectionResponse): UnifiedPrediction[] {
    const unifiedObjectDetection: UnifiedObjectPrediction[] = objects.map((item) => {
        return {
            type: 'OBJECT_PREDICTION',
            data: {
                class: item.class,
