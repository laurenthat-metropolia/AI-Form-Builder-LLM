          ...
          data: {
            ...
            checkboxes: {
              updateMany: {
                ...
                data: body.checkboxes.map((checkbox) => {
                  const existingCheckbox = existingForm.checkboxes.find(
                    (cb) => cb.order
                  );
                  return {
                    id: checkbox.id,
                    order:
                      checkbox.order !== undefined
                        ? checkbox.order
                        : existingCheckbox,
                  };
                }),
            ...
            textFields: {
              updateMany: {
                ...
                data: body.formTextFields.map((textField) => {
                  const existingTextField = existingForm.textFields.find(
                    (tf) => {
                      tf.formId;
                    }
                  );
            ...
            toggleSwitches: {
              updateMany: {
                where: {
                  id: {
                    in: body.formToggleSwitches.map(
                      (toggleSwitch) => toggleSwitch.id
                    ),
                  },
                },
